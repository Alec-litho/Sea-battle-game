...
class GameLogic {
    ...
    public appendShip(ship:Ship):void {
    ...
    public attackShip(y:number,x:number):boolean {
            if(this.map[y][x] === 2) {
                this.existingShips.forEach((ship:Ship) => {
    ...
    public checkForShip(y:number,x:number):boolean {
    ...
    public checkForBarriers(cords:string, shipType:ShipType):boolean {
        let shipTypeLen = shipType.type.length
        ...
        for (let y = +startCord[0]; y <= +endCord[0]; y++) {
            for (let x = +startCord[1]; x <= +endCord[1]; x++) {
                console.log(y, x);
                if(this.map[y][x] === 2) {console.log('barrier');return true }
            }
        }
    }
    public removeShip(shipId:number) {
        let shipIndx:number;
        this.existingShips.forEach((ship, indx) => {if(ship.id === shipId) shipIndx = indx})
        this.existingShips = this.existingShips.splice(shipIndx,1)
    }
    public createShip(x:number,y:number,ship:ShipType) {
        let createdShip = new Ship(x,y,ship)
